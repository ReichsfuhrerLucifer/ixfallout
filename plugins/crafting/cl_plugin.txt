...
function PANEL:Init()
	...
	self:SetSize(ScrW() / 2.4, ScrH() / 1.5)
	self:Center()
	self.infotext = self:Add("DLabel")
	self.infotext:Dock(TOP)
	self.infotext:SetContentAlignment(5)
	self.infotext:SetText("자세한 정보를 보기 위해서는 마우스 커서를 아이콘 위에 가져가세요, 제작 또는 조합을 시도하려면 왼쪽 클릭하세요.")
	...
	for k, v in pairs(STORED_RECIPES) do
		...
		if v["blueprint"] then
			...
			if table.HasValue(data, v["blueprint"]) then
				local item = self:AddItem(v["name"], v["model"], v["desc"], v["req"], v["results"], v["skill"], v["blueprint"], v["entity"])
				item.id = v["id"]
			end
		end
		if not v["blueprint"] then
			local item = self:AddItem(v["name"], v["model"], v["desc"], v["req"], v["results"], v["skill"], v["entity"])
			item.id = v["id"]
		end
...
function PANEL:AddItem(name, icon, desc, req, results, skill, blueprint, entity)
	--[[-------------------------------------------------------------------------
	Structure:
	1: Item Name
	2: Icon
	...
	self.test:SetItem(name, icon, desc, req, results, skill, blueprint or false, entity or false)
...
function PANEL:SetItem(name, icon, desc, req, results, skill, blueprint, guns, entity)
	...
	self.button:SetHelixTooltip(function(tooltip)
		...
		requirements:SetText("재료: " .. table.concat(realreq, ", "))
		local missing = {}
		...
		results:SetText("결과: " .. table.concat(realres, ", "))
		...
		if self.skill then
			...
			skill:SetText("요구 능력: " .. table.concat(skillist, ", "))
			...
			local skill = tooltip:AddRow("skill")
			skill:SetText("이 아이템을 만드는 데 필요한 재료나 능력이 부족합니다.")
			skill:SizeToContents()
		end
...
hook.Add("CreateMenuButtons", "ixCrafting", function(tabs)
	...
	tabs["제작 및 조합"] = function(container)
