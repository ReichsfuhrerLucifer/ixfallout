...
if SERVER then
	netstream.Hook( "ix_SubmitNote", function( client, data )
		...
		ix.log.AddRaw(client:Name() .. " wrote " .. data[1])
	end)

	function playerMeta:OpenNote( text, entity, private )
		netstream.Start( self, "ix_PushNote", {text, entity:EntIndex(), private})
	end
...
if CLIENT then
	...
	function PLUGIN:DrawTargetID(entity, x, y, alpha)
		if (entity:GetClass() == "ix_paper") then
			...
			ix.util.drawText(x, y, L"note_name", color)
				y = y + ix.config.targetTall
				local text = L"note_desc"
			ix.util.drawText(x, y, text, Color(255, 255, 255, alpha))
		end
	end
	
	function ix_OpenNote( text, entity, private )
		...
		function note:viewmode()
			...
			timer.Simple( 0.05, function()
				note.viewer:SetFontInternal( "ixChatFont" )
		...
		function note:editmode()
			...
			note.editor:SetFont( "ixChatFont" )
	...
	function ix_SubmitNote( text, entity )
		netstream.Start("ix_SubmitNote", {text, entity})
	end
	
	netstream.Hook( "ix_PushNote", function( data )
		local text = data[1]
		local ent = data[2]
...
/*
	...
	function PANEL:EditMode()
		...
		self.editor:SetFont( "ixChatFont" )
	...
	function PANEL:ViewMode()
		...
		timer.Simple( 0, function()
			self.viewer:SetFontInternal( "ixChatFont" )
	...
	vgui.Register( "ix_Note", PANEL, "Panel" )

	shit = vgui.Create( "ix_Note" )
	shit:Center()
*/